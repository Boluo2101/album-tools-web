...
// Watchs
watch(() => props.checkedAllStatus, (value) => {
...
<template>
  <div class="Gallery">
    <VirtualList class="images-box" :data-key="'uuid'" :data-sources="getPicturesGroup" :keeps="getKeeps" :estimate-size="getEstimateSize" ref="virtualList" item-class="images">
      <template v-slot="{ source }">
        <ImageCard v-for="(image, index) of source.images" :key="image.uuid" :imageSize="props.imageSize" :withCheckbox="true" :checked="picturesStore.getCheckedGalleryPictures.has(image.uuid)" :details="image" @checkChange="handleImageCheckChange" @mouseEnter="handleImageMouseEnter">
        </ImageCard>
      </template>
    </VirtualList>
