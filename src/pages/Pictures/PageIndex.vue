...
// Components
import { CameraOutlined, DeleteOutlined, FileOutlined, AppstoreOutlined, SwapOutlined, CheckSquareOutlined, BorderOutlined, DownOutlined, FileAddOutlined, FileSyncOutlined, FontSizeOutlined, ArrowDownOutlined, ArrowUpOutlined, PicRightOutlined, PicCenterOutlined } from '@ant-design/icons-vue'
...
const getShowDelete = computed(() => {
  return picturesStore.checkedPictures[getActiveTabKeyName.value].size
})

const getActiveTabKeyName = computed(() => {
  return tabs.value.find(i => i.key === activeKey.value)?.keyName || ''
})
...
<template>
  <div class="PicturesPage">
        ...
        <a-dropdown overlayClassName="PicturesPage">
          <template #overlay>
            <a-menu class="menu" @click="handleImageSizeMenuClick">
              <a-menu-item key="small" :class="['item', imageSize === 'small' ? 'active' : '']">
                小
              </a-menu-item>
              <a-menu-item key="middle" :class="['item', imageSize === 'middle' ? 'active' : '']">
                中
              </a-menu-item>
              <a-menu-item key="large" :class="['item', imageSize === 'large' ? 'active' : '']">
                大
              </a-menu-item>
            </a-menu>
          </template>
        ...
        <a-dropdown overlayClassName="PicturesPage">
          <template #overlay>
            <a-menu class="menu" @click="handleSortMenuClick">
              ...
              <a-menu-divider />
              <a-menu-item key="down" :class="['item', sortType === 'down' ? 'active' : '']">
                <ArrowDownOutlined class="icon" /> 降序
              </a-menu-item>
              <a-menu-item key="up" :class="['item', sortType === 'up' ? 'active' : '']">
                <ArrowUpOutlined class="icon" /> 升序
              </a-menu-item>
            </a-menu>
          </template>
          <a-button type="text">
    ...
    <!-- gallery -->
    <Gallery v-show="activeKey === 1" :sortBy="sortBy" :sortType="sortType" :showImageView="showImageView" :imageSize="imageSize" :checkedAllStatus="checkedAllStatus" @checkAllStatusChange="handleCheckAllStatusChange" @checkedImages="handleCheckedImages" />

    <!-- remove similar pictures -->
    <SimilarImages v-show="activeKey === 2" :sortBy="sortBy" :sortType="sortType" :showImageView="showImageView" :imageSize="imageSize" @checkedImages="handleCheckedImages"></SimilarImages>
  </div>
</template>
