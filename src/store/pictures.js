  ...
  actions: {
    ...
    setPictures(value, cacheName = "/") {
      ...
      this.pictures = value.map((i) => {
        ...
        const pictureDetails = this.picturesDetails.get(i.path) || {}
        this.picturesDetails.set(i.path, { ...details, ...pictureDetails })
    ...
    setPicturesDetails(array) {
      array.forEach((i) => {
        const pictureDetails = this.picturesDetails.get(i.path) || {}
        this.picturesDetails.set(i.path, { ...pictureDetails, ...i })
        // 以下需要优化
        // const dbDetails = i
    ...
    async moveToTrash(params) {
      ...
      const uuidsByPaths = params.paths.map((i) => this.picturesDetails.get(i).uuid)
      ...
      params.paths.forEach((i) => {
        this.pictures = this.pictures.filter((j) => j.path !== i)
